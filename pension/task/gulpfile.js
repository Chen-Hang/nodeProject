...
gulp.task('sprite',function(){
	return gulp.src('../public/iamges/icons/**.jpg')
	.pipe(spritesmith({
		imgName: 'sprite.png',
		cssName: 'sprite.css'
	}))
	.pipe(gulp.dest('../public/dist/img/icons'));
})
...
gulp.task('server', function () {
	...
	//     files:['**'],
	//     server:{
	//         baseDir:'../public', // 设置服务器的根目录
	//         // index:'../views/index.html' // 指定默认打开的文件
	//     },
	...
	gulp.watch(src.sass,function(event){
		var paths = watchPath(event, '../public/stylesheets/', '../public/dist/css/');
		gulp.src(paths.srcPath)
		.pipe(sass({outputStyle:'compressed'}).on('error', sass.logError))
		// .pipe(sass())
		.pipe(prefix())
	...
	gulp.watch(src.js,function(event){
		var paths = watchPath(event, '../public/javascripts/', '../public/dist/js/'),
		distDir = paths.distDir;
			gulp.src(paths.srcPath)
			.pipe(babel())
			// 这会输出一个压缩过的并且重命名为 foo.min.js 的文件
	...
	gulp.watch(src.img,function(event){
		var paths = watchPath(event, '../public/iamges/', '../public/dist/img/'),
		distDir = paths.distDir;
		gulp.src(paths.srcPath)
		.pipe(imagemin())
    	.pipe(gulp.dest(distDir));
...
gulp.task('build', ['convertJS', 'convertCSS', 'convertSass','convertHTML', 'convertIMG']); //moveLib
// 实时刷新监听
